...
export function SupabaseDocs() {
  ...
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || "https://hhrwxfrmavcufmedxwwy.supabase.co";
  const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || "sb_publishable_YlHMsda0URfE5Kpbt2TVbQ_dX8kk1nX";
...
create policy "HR administrators can manage all departments" on public.departments for all using (auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role');
create policy "Allow employees to inspect their own 201 profile" on public.employees for select using (email = auth.jwt() ->> 'email' or auth.role() = 'service_role');
create policy "HR administrators can manage all employee profiles" on public.employees for all using (auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role') with check (auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role');
create policy "Employees can inspect and register their own attendance logs" on public.attendance_logs for all using (employee_id = (select id from public.employees where email = auth.jwt() ->> 'email') or auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role') with check (employee_id = (select id from public.employees where email = auth.jwt() ->> 'email') or auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role');
create policy "Employees can manage their own leave requests" on public.leave_requests for all using (employee_id = (select id from public.employees where email = auth.jwt() ->> 'email') or auth.role() = 'service_role') with check (employee_id = (select id from public.employees where email = auth.jwt() ->> 'email') or auth.role() = 'service_role');
create policy "HR administrators can review and approve all leave requests" on public.leave_requests for all using (auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role');
create policy "Employees can manage their own overtime requests" on public.overtime_requests for all using (employee_id = (select id from public.employees where email = auth.jwt() ->> 'email') or auth.role() = 'service_role') with check (employee_id = (select id from public.employees where email = auth.jwt() ->> 'email') or auth.role() = 'service_role');
create policy "HR administrators can review and approve all overtime requests" on public.overtime_requests for all using (auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role');
create policy "Employees can read and manage their own itineraries" on public.itineraries for all using (employee_id = (select id from public.employees where email = auth.jwt() ->> 'email') or auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role') with check (employee_id = (select id from public.employees where email = auth.jwt() ->> 'email') or auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role');
create policy "Employees can issue correction requests" on public.maker_requests for all using (requester_email = auth.jwt() ->> 'email' or auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role') with check (requester_email = auth.jwt() ->> 'email' or auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role');
create policy "Only HR administrators can manage payroll runs" on public.payroll_runs for all using (auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role');
create policy "Only system or admin can inspect audit logs" on public.audit_logs for all using (auth.jwt() ->> 'email' like '%@corp.ph' or auth.jwt() ->> 'email' = 'gmelchan951@gmail.com' or auth.role() = 'service_role');
  `.trim();
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
        ...
        <p className="text-gray-500 text-xs leading-relaxed mb-6">
          Integrate Supabase Postgres to synchronize live employee state with RLS (Row Level Security) protection. Copy the complete optimized DDL schema block below and paste it into the Supabase SQL Editor console.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div className="p-4 bg-[#1E3A5F]/5 border border-[#1E3A5F]/10 rounded-lg">
            ...
            <p className="text-[#1E3A5F] text-xs leading-normal">
              Employees can view only their own records based on JWT credentials, while HR Administrators with email domains ending in <strong>@corp.ph</strong> (or <strong>gmelchan951@gmail.com</strong>) have complete read/write access.
            </p>
          </div>
        </div>
